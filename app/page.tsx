  ...
  return (
    <main>
      ...
      <section className="focus-section" id="focus"><div className="section-shell"><details className="section-fold" open>
        ...
        <div className="section-fold-content"><div className="focus-table" data-reveal data-motion><div className="focus-table-head"><span>问题 / Problem</span><span>方法 / Method</span><span>证据 / Evidence</span><span>产出 / Output</span></div>{focusAreas.map((item) => { const Icon = item.icon; return <article className="focus-row" key={item.title}><div className="focus-problem"><span>{item.index}</span><Icon aria-hidden="true" /><div><h3>{item.title}</h3><p>{item.question}</p></div></div><ul className="focus-methods" aria-label="研究方法">{item.method.map((method) => <li key={method}>{method}</li>)}</ul><ul className="focus-evidence" aria-label="验证证据">{item.evidence.map((proof) => <li key={proof}>{proof}</li>)}</ul><div className="focus-output"><span>形成产出</span><strong>{item.output}</strong></div></article>; })}</div></div>
      </details></div></section>
        ...
        <div className="section-fold-content">
          <article className="product-card" data-reveal data-motion data-glow>
            ...
            <div className="product-copy">
              ...
              <a className="product-link" href="https://ai-resets.com/" {...externalLinkProps}><Image className="product-link-logo" src="/ai-resets-icon.png" alt="" width={38} height={38} /><span>ai-resets.com</span><PiArrowUpRightBold aria-hidden="true" /></a>
            </div>
          </article>
        </div>
      </details></div></section>
